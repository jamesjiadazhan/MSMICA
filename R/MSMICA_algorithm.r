...
MSMICA_algorithm <- function(met_raw_wide, metabolite_identified, identified_target = NULL, Adduct = c("M+H","M+2Na-H","M+Na","M-H2O+H","M+K","M+2H"), prefix="", ion_mode="positive", detail=FALSE, adduct_correlation_r_threshold=0.4, adduct_correlation_time_threshold=5, isotopic_correlation_r_threshold=0.5, isotopic_correlation_time_threshold=5, mean_absolute_isotope_ratio_deviation_threshold=5) {
    ...
    KEGG_database_2_mz = mass2mz_df(mass=KEGG_database_2$Exact_mass, adduct=KEGG_database_2$Adduct)
    ...
    KEGG_database_2_mz_heavy = mass2mz_df(mass=KEGG_database_2$isotopic_mass, adduct=KEGG_database_2$Adduct)
    ...
    bayesian_probability_calculation_cluster = function(data, MSMICA_cor_input) {
        ...
        if(nrow(data) > 1) {
            ...
            full_cor_matrix_annotated <- cor(MSMICA_cor_input_annotated, MSMICA_cor_input_annotated, method="spearman", use="complete.obs")
            ...
            # result_group <- do.call(rbind, lapply(unlist(intra_correlation_all, recursive = FALSE), check_time_range_group))
        ...
        if (!all(is.na(data_adduct_corr_3$adduct_corr_cluster))){
            ...
            if (detail == TRUE){
                ...
                full_cor_matrix_annotated_final <- cor(MSMICA_cor_input_annotated_final, MSMICA_cor_input_annotated_final, method="spearman", use="complete.obs")
    ...
    while(nrow(annotated_metabolite) > 0) {
        ...
        # Convert data frames to data tables if not already converted
        identified_metabolite <- as.data.table(identified_metabolite)
        kegg_connection <- as.data.table(kegg_connection)

        # Set keys for faster joins
        setkey(identified_metabolite, KEGGID)
        setkey(kegg_connection, connection_1)

        # left join the kegg_connection database to the MSMICA_TCA_col_names_identified database
        MSMICA_col_names_connection <- identified_metabolite[kegg_connection, nomatch = 0, allow.cartesian = TRUE]

        # Arrange the data by the connection_2 column in ascending order 
        setorder(MSMICA_col_names_connection, connection_2)

        # convert MSMICA_col_names_connection_simplifed from data.table to tibble
        MSMICA_col_names_connection_simplifed_2 = tibble(MSMICA_col_names_connection)
        ...
        if (nrow(final_results) > 0) {
            ...
            print(unique(final_results$connection_2))
            ...
            identified_metabolite = rbind(identified_metabolite, final_results_2, fill=TRUE) %>%
